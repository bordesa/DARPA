...
def gen_feature_nodearray(xi, feature_max=None, issparse=True):
	...
	elif isinstance(xi, numpy.ndarray): #CHANGE HERE
		xi=numpy.append([0],xi)
		index_range = range(1, xi.shape[0])# idx should start from 1
	else:
		raise TypeError('xi should be a dictionary, list, tuple or numpy.array')
...
class problem(Structure):
	...
	def __init__(self, y, x, bias = -1):
		...
		for i, yi in enumerate(y): self.y[i] = int(y[i])# CHANGE HERE
